# Android In-App Update — Backend Integration (svid + vidcombo)

**Scope:** Contract between the Android clients (svid + vidcombo, `svid-android` repo) and **this Go backend** (`snakeloader/backend`).
...
The Android apps (svid + vidcombo) previously checked for updates against two
team-external PHP endpoints:
- `https://api.svid.app/app_version.php` → **HTTP 404** (route dead)
...
```
GET /api/v1/updates/check?platform=android&version={versionName}&channel=stable&brand={svid|vidcombo}
```
...
- `channel=stable`
- `brand=svid` or `brand=vidcombo` — sent explicitly by Android.
...
|----------|---------------------------------------------|
| svid    | `https://api.svid.app/api/v1/updates/check` |
| vidcombo | `https://api.svid.app/api/v1/updates/check?brand=vidcombo` |

The Go backend lives **only at `api.svid.app`**. Brands are differentiated by the
`?brand=` query param, not by host — exactly how desktop does it
(`brand_config.dart` → `goBackendBaseUrl` is `api.svid.app/api/v1` for *both*
...
> must point to `https://api.svid.app/api/v1/updates/check` (with
> `brand=vidcombo`), **not** `https://api.vidcombo.net/...`. Pointing it at the
> PHP host will 404. (This is a change in `svid-android` `app/build.gradle`,
...
```json
{
  ...
  "data": {
    ...
    "is_mandatory": false,
    "release_notes": "…",
    "download_url": "https://…/svid-1.6.0.apk",
    "file_size": 50401069,
    "checksum": "<sha256 hex>",
...
1. **`download_url` MUST be a direct `.apk` file.** Android's `DownloadManager`
   ...
   release asset / CDN), mirroring how desktop distributes via `svid-releases`.
2. **Per-brand, correctly-signed APK.** svid APK signed with the SVID release
...
|-----|---------------------------------|--------|
| svid-android    | `1.6.0` | `app/build.gradle:58` |
| vidcombo-android | `5.0.0` | `app/build.gradle:116` |
| svid-desktop    | `1.3.x` | (different bucket) |
...
- `brand=vidcombo, platform=android` → version must be **≥ 5.x** (NOT 1.7.x from
  desktop vidcombo).
- `brand=svid, platform=android` → version must be **> 1.6.0**.
...
1. Register a test Android release (version higher than installed) for brand `svid`
   (admin `POST /admin/v1/releases` with `platform=android`, or CI
   `POST /internal/ci/releases` with an `android` key in `platforms`).
2. `GET /api/v1/updates/check?platform=android&version=1.5.0&channel=stable&brand=svid`
...
## Android client reference (svid-android repo)
...
- `app/build.gradle` — `UPDATE_API_URL` per flavor (vidcombo must point to `api.svid.app`)
- Full Android-side contract: svid-android `docs/ANDROID_UPDATE_CONTRACT.md`
