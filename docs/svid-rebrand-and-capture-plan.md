...
> copy of `mydinh-studio/svid-desktop`. Old `website/` + vendored AppImage removed.
...
## Workstream A — Full-identity rebrand `svid` → `svid`
...
**Consequences accepted:** existing Svid installs will not auto-update to Svid;
old `svid.db` history is under a new DB name; `SVID-` license keys must be
...
- Multi-brand via `--dart-define=BRAND=svid|vidcombo`. Identity is centralized:
  - `lib/core/config/brand_config.dart` — `enum Brand`, `SvidBrand` class, all identity strings.
  - `macos/Runner/Configs/brands/svid.xcconfig` — macOS bundle identity.
  - `windows/runner/brand_config.h` (generated) + `scripts/set_brand.sh`.
  - `assets/brands/svid/` — logos/icons/tray.
...
**Stage 1 — Dart package rename** (`svid` → `svid`)
- `pubspec.yaml`: `name: svid` → `name: svid`; description.
- Rewrite 298 files: `package:svid/` → `package:svid/` (imports/exports).
...
- `enum Brand { svid, vidcombo }` → add/rename `svid`; update `fromString` default,
  `_resolve` switch, and every `Brand.svid` reference in `lib/` (218 hits/65 files).
- `SvidBrand` → `SvidBrand`; `appName 'Svid'→'Svid'`; `databaseName 'svid'→'svid'`
  (⚠️ never add `.db` — see c8bbba91 regression assertion); `urlScheme 'svid'→'svid'`;
  `bundleId 'com.svid.app'→'com.svid.app'`; `windowsAppUserModelId`; `methodChannelPrefix`;
  backend URLs `api.svid.app`→`api.svid.app`; `websiteUrl`→`https://svid.app`;
  `versionCheckUrl`→`https://svid.app/version.json`; `backendAppName 'appSvid'→'appSvid'`.
- License: `SVID-` pattern → `SVID-`; hint/example strings. **Keep backward-compat**:
  in `isValidLicenseKey`, also accept legacy `SVID-...` keys (mirror VidCombo's pattern).
- Default `BRAND` dart-define fallbacks `'svid'` → `'svid'`.
- Verify: `fvm flutter analyze`; run brand-config unit tests.

**Stage 3 — Platform configs**
- macOS: rename `macos/Runner/Configs/brands/svid.xcconfig` → `svid.xcconfig`; update
  `PRODUCT_BUNDLE_IDENTIFIER`, product/app name `svid.app`→`svid.app`, `com.example.svid`
...
- Assets: `git mv assets/brands/svid assets/brands/svid` (+ any icon filenames).
- `scripts/set_brand.sh`: add/rename the `svid` case; update every build/package script
  (`package_*`, `release.sh`, `preflight_release.sh`) that hardcodes `svid`.
- Verify: `scripts/set_brand.sh svid` then `fvm flutter build macos/windows --dart-define=BRAND=svid`.

**Stage 4 — Native (Rust)** (`native/`, 4 hits) — grep + fix any `svid` string/path; `cargo check`.

**Stage 5 — Go backend** (`backend/`, 338 hits/87 files) — SEPARATE service:
- go module path if it embeds `svid`; `api.svid.app`→`api.svid.app`; device app name
  `appSvid`→`appSvid`; license `SVID-` validation (keep legacy accept); Stripe testdata
  fixtures are fine to leave. Verify: `go build ./...` + `go test ./...`.

**Stage 6 — Docs + tests** (docs 616, test 1086) — lower priority; update `SVID-` license
