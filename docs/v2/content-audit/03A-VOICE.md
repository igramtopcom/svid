...
**Scope**: Áp dụng toàn app SSvid + VidCombo, locale VI + EN. Khi mở rộng thêm 13 locale ở v2.1, file này vẫn là gốc — mỗi locale chỉ adapt rule (capitalization, pronoun) tương ứng, không tạo voice mới.
...
Voice Invariant Principles (§2)            ← cả 2 brand đều theo
    ↓
Brand Voice — SSvid (§3)                   ← warm-confident
...
Mỗi tầng KHÔNG override tầng trên — chỉ refine. Một button SSvid trong dialog warning vẫn:
...
## 3. SSvid Voice — "Warm-Confident"

### 3.1 One-liner

> SSvid sounds like a confident host who happens to love cinema — warm, present, never showy.

### 3.2 Anchor — diễn dịch "Nocturne Cinematic" từ visual sang language

`DESIGN.md` chốt SSvid là "Obsidian Wine Cellar / Nocturne Cinematic" — wine red `#8D021F`, Inter sharp 3px, tonal layering. Đó là **visual mood**, không phải language mood.
...
- ❌ "Hành trình khám phá thế giới video bắt đầu từ đây với SSvid — người bạn đồng hành đáng tin cậy."
...
### 3.4 SSvid tagline (replace "powered by Rust + Flutter")
...
| "Bộ sưu tập của bạn đang chờ. Dán link đầu tiên ở trên." | "Welcome to SSvid! Your journey begins here..." |
...
- **Approachable** (từ visual rounded shape) = không robotic. Có "bạn" pronoun, có câu dễ chịu, nhưng không personal touch như SSvid.
...
- ❌ "Bộ sưu tập của bạn đang chờ. Dán link đầu tiên ở trên." (đây là SSvid)
...
### 4.6 SSvid vs VidCombo — bảng so sánh đặt cạnh nhau

| Surface | SSvid | VidCombo |
|---|---|---|
...
| Snackbar success (default count = 1) | 🟡 Partial | SSvid có thể "✅ Đã tải xong"; VidCombo "Đã tải" |
...
- Brand-specific: `home.subtitle.ssvid`, `home.subtitle.vidcombo` — code resolve via `BrandConfig.current.brand` để pick suffix
...
### 6.1.1 SSvid vs VidCombo — sub-rule pronoun
...
- SSvid **prefer giữ pronoun** ở những chỗ tương đương để duy trì tone warm.
...
**Không**. Cả SSvid và VidCombo dùng cùng "bạn" / "you". Brand voice khác nhau ở **diction + sentence structure**, không ở pronoun. Pronoun = system-level rule.
...
2. Danh từ riêng (YouTube, TikTok, SSvid, VidCombo, Cài đặt khi là menu name riêng — debatable).
...
| Quota banner | ❌ No | (Ngoại lệ Premium upsell SSvid có thể `✨` đầu — debatable, em recommend bỏ) |
...
**Lý do**: SSvid voice = warm, emoji 1-icon đầu chuỗi acceptable. VidCombo voice = utility-clean, không emoji. → emoji là 1 trong những cách 2 brand differentiate.
...
- ❌ "Welcome to SSvid!" → ✅ "Welcome to {appName}!"
- ❌ `'Downloads/SSvid'` (preset_popover.dart leak) → ✅ `'Downloads/${BrandConfig.current.appName}'`
...
|---|---|---|
| Home empty (default) — SSvid | "Bộ sưu tập của bạn đang chờ" | "Dán link đầu tiên ở trên" |
...
| Step | SSvid title | SSvid body | VidCombo title | VidCombo body |
|---|---|---|---|---|
...
|---|---|---|---|
| SSvid | Còn lượt | "Bạn còn {count} lượt tải hôm nay" | "Nâng cấp" |
| VidCombo | Còn lượt | "Còn {count} lượt hôm nay" | "Nâng cấp" |
| SSvid | Hết lượt | "Đã hết lượt hôm nay. Nâng cấp để tải tiếp." | "Nâng cấp" |
| VidCombo | Hết lượt | "Hết lượt hôm nay. Nâng cấp." | "Nâng cấp" |

### 12.10 Premium upsell

**SSvid voice**: emotional appeal, value framing.
**VidCombo voice**: feature list, direct.

| Brand | Title | Subtitle | CTA |
|---|---|---|---|
| SSvid | "Mở khóa toàn bộ thư viện" | "Tải không giới hạn. 4K. Hàng loạt. Mãi mãi." | "Nâng cấp" |
...
- Banned vocabulary: same SSvid list + no aspirational lines.
...
3. Brand-specific?
   → tra §5.2 — surface nằm trong 30% brand-specific list?
       Yes → viết 2 version (SSvid + VidCombo) hoặc dùng BrandConfig getter.
...
| V3 | SSvid tagline | ✅ Đổi sang B | "Tải video. Đơn giản. Đẹp." chốt §3.4 |
...
| V6 | Pronoun | ✅ "bạn" cả 2 + nuance | §6.1.1 VidCombo selective drop, SSvid keep |
| V7 | VI sentence case sweep | ✅ Yes | — |
| V8 | Emoji policy | ✅ SSvid selective / VidCombo none | Pass 03C: snackbar accept icon component (eng task) |
