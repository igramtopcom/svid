...
**Spec đọc**: `docs/Svid_v2_Design_Spec.md` v1.2 (1322 dòng), tập trung §0-§8 (foundation tokens, ~570 dòng)
...
4. **🟠 Multi-brand gap đã có tone trong code, KHÔNG có trong spec**: BrandConfig đã 100% multi-brand-aware (Svid Wine Red + dark default + 3px radius + Inter; VidCombo Arctic Blue + light default + 12px card radius / 999px button radius + DM Sans). Spec V2 chỉ nói Svid → áp dụng tokens spec sẽ phá VidCombo. → Phải resolve ở Pass 2E.
...
|---|---|---|---|
| §2.0 Brand identity (Wine Red `#8D021F` source of truth) | MUST | `BrandConfig.SvidBrand.colors.brand = #8D021F` ✓ | ✅ Match |
...
| §3.1 Font stack: Inter bundled `assets/fonts/InterVariable.ttf` weights 100-900, fallback `-apple-system/Segoe UI Variable/Cantarell/system-ui` | **MUST** | **🔴 Inter PHANTOM**: `pubspec.yaml fonts:` section L171-180 COMMENTED OUT (sample skeleton từ Flutter init). `assets/fonts/InterVariable.ttf` mới pull về turn này (chưa wired). `google_fonts ^6.3.2` dependency vẫn ở L110 nhưng `main.dart:48` set `allowRuntimeFetching=false`. App Svid render với **system fallback**, không phải Inter bundle. | 🔴 Pass 2A Block #1 |
...
|---|---|---|
| §5 Radius: `none 0 / xs 2 / sm 4 / md 8 (default) / lg 12 / xl 16 / 2xl 24 / round 999` | `BrandConfig.cardRadius/buttonRadius/inputRadius/dialogRadius/chipRadius/popupRadius` — Svid all `3` (Nocturne angular), VidCombo varied (`12 / 999 / 8 / 12 / 999 / 8`). **Conflict**: spec default = 8 (`md`), code Svid = 3 toàn bộ | 🔴 **Brand intent conflict** — spec áp dụng Svid sẽ phá Nocturne Cinematic angular identity |
| §6 Shadow elevation (`sm/md/lg/xl/2xl`, layered Material 3, dark 1.5× opacity) | `BrandConfig.glowSubtle/glowIntense/glowCta` cho premium accents. KHÔNG có generic `AppShadow` class. Svid `cardElevation = 0` (flat), VidCombo `cardElevation = 2` | 🟠 Tạo `AppShadow` không phá brand elevation policy |
...
| Token group | Svid current | VidCombo current | V2 spec impact |
|---|---|---|---|
| Brand color | Wine Red `#8D021F` | Arctic Blue `#0066CC` | Spec say "Wine Red". Áp dụng nguyên xi sẽ phá VidCombo. **Resolution**: V2 spec chỉ apply lên Svid — VidCombo continues qua BrandConfig override. |
| Font family | Inter (phantom hiện tại) | DM Sans (phantom hiện tại — DM Sans cũng KHÔNG bundled) | Cả 2 brand đều phantom font. V2 ship Inter bundle cho Svid. **Câu hỏi cho Chairman**: VidCombo nên giữ DM Sans hay unify Inter? DM Sans có ý nghĩa branding "Arctic Command geometric" — nếu unify Inter sẽ mất brand differentiation. |
...
| Theme mode default | dark (Nocturne Cinematic) | light (Arctic Clarity) | Mockup ảnh anh Kỳ là **light mode**. Spec không nói. → Confirm: dark default cho Svid V2 vẫn giữ? |
...
- macOS 14+ có Inter pre-installed → app Svid trên macOS render đúng Inter (system, không phải bundle)
...
| Q4 | **Default theme mode Svid V2 = dark hay light?** | Mockup là light. Svid hiện default `ThemeMode.dark` (Nocturne). **Đề xuất**: giữ dark default, mockup chỉ là 1 mode demo — cả 2 mode phải support đầy đủ (a11y). |
| Q5 | **Card radius mâu thuẫn (spec default 8px vs Svid 3px Nocturne angular)** — apply theo brand hay theo spec? | **Brand wins** cho brand-defining components (cards, buttons, inputs) — đây là Nocturne identity. Spec 8px chỉ cho NEW generic components không brand-specific (vd: empty state illustrations, generic dialogs). |
