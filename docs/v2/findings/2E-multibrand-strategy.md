...
1. **🔴 Reality check**: V2 specs (3 doc) **HOÀN TOÀN SSvid-only**. ZERO mention VidCombo, Arctic Blue, DM Sans, 10-quota. Thực thi nguyên xi sẽ phá VidCombo (50k devices, 25× SSvid user base).
...
3. **🟢 EM CHỌN strategy (D) HYBRID**: V2 ship **SSvid first** (v2.0.0), VidCombo continues **v1.x home** với feature flag brand-conditional. VidCombo migrate v2 ở **release cycle kế tiếp (v2.1.0)** sau SSvid stable.
    - Lý do: Spec author (anh Kỳ) intent SSvid-first; respect requirement. Risk control: VidCombo 50k users không risk regression. QA scope manageable (1 brand at a time). Code path identical, chỉ flag toggle.
    - **CTO autonomous decision** — em không cần Chairman approve, but anh có thể veto.

4. **🟠 6 multi-brand work surface phải address khi triển khai SSvid V2**:
    ...
    - Theme mode default check (SSvid dark → V2 layout; VidCombo light → existing v1 home, no V2 yet)
    - Card radius respect BrandConfig (3px SSvid vs 12px VidCombo — V2 components must use `BrandConfig.current.cardRadius` không hardcode)
...
5. **🟢 Risk register update**: 4 new rows (Pass 2C đã add 4, Pass 2E formalize):
    ...
    - V2 spec hardcoded SSvid copy leak
...
| Surface | SSvid (Nocturne Cinematic) | VidCombo (Arctic Command) | V2 spec impact |
|---|---|---|---|
...
| Font family | `Inter` (phantom — pubspec not wired) | `DM Sans` (phantom — also not wired) | 🔴 Both phantom currently. SSvid V2 fixes Inter; VidCombo DM Sans fix deferred |
...
| License key pattern | `SSVID-XXXX-...` | 32 hex OR `VIDCOMBO-XXXX-...` | Backend layer — V2 không touch |
| Backend type | Go (api.ssvid.app) | PHP (api.vidcombo.net) | Backend layer — V2 không touch |
| Free daily quota | 15 | 10 | Spec §7 hardcode "15" — MUST refactor `BrandConfig.freeDailyDownloads` getter |
| Bundle ID | `com.ssvid.app` | `com.tinasoft.vidcombo` | OS layer — V2 không touch |
...
|---|---|---|---|---|
| **A** SSvid-only ship v2.0.0 | Spec compliance, VidCombo continues v1.x indefinitely | Fast, spec aligned | VidCombo never gets V2 — bad long-term | ❌ |
| **B** Song song ship | V2 cả SSvid + VidCombo cùng v2.0.0 | Single release cycle | 2× QA, 2× regression risk, spec doesn't cover VidCombo | ❌ |
...
| **D** **HYBRID** | SSvid v2.0.0 first (3 weeks dev). VidCombo v2.1.0 after SSvid stable (+1 cycle, ~2 weeks port) | Risk control, spec-aligned, sustainable | VidCombo waits 1 cycle | ✅ **Recommended** |

**Strategy (D) timeline**:
```
Week 1-7  : V2 dev SSvid only (per Pass 2C realistic 36-37d single dev)
            VidCombo continues v1.x home — feature flag homeV2Enabled
            brand-conditional defaults (SSvid=true, VidCombo=false)
Week 8    : SSvid v2.0.0 ship public
Week 9-10 : SSvid v2.0 stability monitor, hotfix if needed
...
## 4. SSvid V2 cycle — multi-brand audit per phase

### 4.1 Phase 0 (Foundation)

| Task | Multi-brand consideration |
|---|---|
| Wire pubspec `fonts:` Inter | 🟢 SSvid only. VidCombo (DM Sans) deferred to v2.1 cycle. NOTE: VidCombo build hiện tại cũng phantom DM Sans — không degrade. |
...
| Task | Multi-brand consideration |
|---|---|
| `SmartInputBar` widget | 🟢 Use `colorScheme.primary` (auto Wine Red SSvid / Arctic Blue VidCombo) |
...
| Inter font bundle | 🟢 SSvid only (VidCombo deferred) |
...
| Task | Multi-brand consideration |
|---|---|
| 9 row state colors | 🟢 Use AppColors which delegates BrandConfig (warm rose for SSvid, cool blue for VidCombo) |
| Filter popover | 🟢 OK |
| Sort dropdown | 🟢 OK |
| Tag chips | 🟢 OK |
...
| Task | Multi-brand consideration |
|---|---|
| DB v18→v19 UserPlaylists tables | 🟢 Same schema both brands (database file `ssvid.db` vs `vidcombo.db` separate per BrandConfig.databaseName) |
...
| WCAG AA contrast verify | 🔴 Phải verify cả 2 brand (SSvid dark Wine Red text contrast vs VidCombo dark Arctic Blue text contrast different) — but VidCombo deferred to v2.1, so SSvid only this cycle. |
...
| What's new dialog | 🟠 Copy may differ per brand — but VidCombo doesn't get V2 yet, so SSvid-specific copy OK. |

**Polish multi-brand impact: nil for SSvid v2.0 cycle.**

### 4.8 Buffer phase

| Task | Multi-brand consideration |
|---|---|
| Manual QA | 🔴 **MUST run BOTH brands**: SSvid (V2 home) + VidCombo (v1.x home, verify feature flag works, no regression). |
| Migration smoke test | 🔴 Both brands DB upgrade v18→v19 (UserPlaylists tables exist but unused for VidCombo until v2.1) |
| Internal dogfooding | 🟢 SSvid only |
| Bug triage | 🟢 SSvid only |
| Release notes | 🟢 SSvid only |
...
| Flag | Default value SSvid | Default value VidCombo | Lifecycle |
|---|---|---|---|
| `homeV2Enabled` | `false` (alpha→beta→public) | `false` (always until v2.1 port) | SSvid: flip to `true` at public release. VidCombo: stays `false` until v2.1. |
| `playlistContextEnabled` | `false` (until §10 ships) | `false` (always until v2.1) | SSvid: flip to `true` after Phase §10 + integration tests pass. |
...
class FeatureFlags {
  static bool get homeV2Enabled {
    ...
    // SSvid: respect remote flag from backend OR local override
    return _remoteFlag('home_v2_enabled') ?? _kSSvidV2DefaultEnabled;
  }
}
```

Rollback path: `_kSSvidV2DefaultEnabled = false` flips entire SSvid back to v1.x home in 1 hotfix release.

---

## 6. VidCombo V2 cycle (v2.1.0) — pre-spec'd plan

When SSvid V2 stable (~week 9), em propose:

### 6.1 Tasks (~6d total)

| # | Task | Effort |
|---|---|---|
| 1 | Branch `feat/v2-vidcombo-port` from SSvid v2.0 stable tag | 0.25d |
...
| 7 | Cross-brand smoke test (SSvid + VidCombo on macOS/Windows/Linux) | 1d |
...
|---|---|---|---|
| **Hardcoded copy strings leak SSvid context** (vd "Bạn còn 15..." literal) | High | Medium | Audit Phase 1A for `BrandConfig.freeDailyDownloads` usage, lint rule check no "15" / "10" literals in V2 widgets |
| **VidCombo regression during SSvid V2 dev** (shared core code) | Medium | High | CI gate: build cả 2 brand with `--dart-define=BRAND=ssvid|vidcombo`, smoke test both each PR |
| **VidCombo missing V2 user complaint** | Low | Low | Communicate to anh Kỳ: "VidCombo V2 ships v2.1, ~2 weeks after SSvid v2.0" |
...
| ID | Resolution |
|---|---|
| Q13 (Pass 2C) — Multi-brand release schedule | **Strategy (D) HYBRID**. SSvid v2.0.0 first, VidCombo v2.1.0 +1 cycle (~2 weeks). |
| Q11 (Pass 2C) — Single dev or 2+ dev parallel | Em recommend **2-dev parallel** for SSvid V2 cycle (Track A + B), saves ~12d. VidCombo v2.1 single-dev (small port). |
...
| ✅ | **Strategy (D) HYBRID chosen** — SSvid v2.0 first, VidCombo v2.1 +1 cycle |
