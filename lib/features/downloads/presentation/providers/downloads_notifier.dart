...
class DownloadsNotifier extends StateNotifier<DownloadsState> {
  ...
  Future<RetryDownloadPlan?> _buildRetryPlanFromSettings(
    ...
    // 2026-07) via the shared rule: a 1440p+ MP4 retry on YouTube becomes MKV
    // (native remux) before the plan/selector below so the whole retry derives
    // from MKV. New downloads already carry a .mkv filename (fresh-path fix) so
    // this is belt-and-suspenders; it also upgrades retries of pre-fix rows.
    container = ContainerPlanner.resolveSmartContainer(
      picked: container,
      platform: PlatformDetector.detectPlatform(download.url),
      height: QualityResolutionParser.parseHeight(download.qualityLabel ?? ''),
    );
