...
std::wstring CurrentProcessAppUserModelId() {
  PWSTR appUserModelId = nullptr;
  const HRESULT hr = ::GetCurrentProcessExplicitAppUserModelID(&appUserModelId);
  if (FAILED(hr) || appUserModelId == nullptr) {
    return std::wstring();
  }

  std::wstring result(appUserModelId);
...
void LocalNotifierPlugin::Setup(
  ...
  std::string appUserModelId = appName;
  const auto appUserModelIdIterator =
      args.find(flutter::EncodableValue("appUserModelId"));
  if (appUserModelIdIterator != args.end()) {
    appUserModelId = std::get<std::string>(appUserModelIdIterator->second);
  }
  std::string shortcutPolicy =
      std::get<std::string>(args.at(flutter::EncodableValue("shortcutPolicy")));

  const std::wstring appNameWide = converter.from_bytes(appName);
  std::wstring appUserModelIdWide = converter.from_bytes(appUserModelId);
  const std::wstring processAppUserModelId = CurrentProcessAppUserModelId();
  if (!processAppUserModelId.empty() && processAppUserModelId != appNameWide) {
    appUserModelIdWide = processAppUserModelId;
  }

  WinToast::instance()->setAppName(appNameWide);
  WinToast::instance()->setAppUserModelId(appUserModelIdWide);
